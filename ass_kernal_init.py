...
ways = ['glorot_uniform','identity','ones','random_normal','constant','random_uniform']
# create nn network
# x's size is 990 * 192
for i in ways:
    model = Sequential([
        Dense(512,input_dim=192,kernel_initializer = i, activation='relu'),
        #Dropout(0.3),
        # we need 99 outputs
    ...
    history = model.fit(x = X, y = y_one_hot, batch_size=64,epochs=200,shuffle = True, verbose=0,validation_split=0.2)
...
plt.xlabel("epochs")
plt.ylabel("val_loss")
plt.title("differrent kernel_initializer's val_loss")
plt.legend()
plt.savefig("kernel_initializer's loss.png")
plt.show()
